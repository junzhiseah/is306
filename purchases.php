...
<div class="title">
	<img src="img/Shopping-Cart-02-24.png"/>
	<br/>
	My Purchases
</div>
...
<?php 
	...
	if (sizeof($purchaseItems) > 0) {
		...
		for ($i=0; $i < sizeof($purchaseItems); $i++) { 
			...
			echo '<tr><td class="quantity">'.$quantity.' '.$item['quantifier'].'(s) purchased'.'</td>';			

			
			
			echo '<form action="qrcode.php" method="GET">';
			echo '<input type="hidden" name="itemid" value="'.$itemId.'" />';
			echo '<td><input type="submit" value="Redeem" /></td></tr>';
			echo '</form>';
			echo '<tr class="space"><td></td><td></td></tr>';
		}
		echo '</table>';
	} else {
		echo '<h3 class="nopurchases">'.'You have not made any purchases at the moment.'.'</h3>';
	}
?>
